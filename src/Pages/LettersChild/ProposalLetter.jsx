...
import { Box, Button, Chip, Radio, TextField } from '@mui/material';
...
const ProposalLetter = () => {
  ...
  const { proposalList, getProposal } = useProposalList();
  const { loading, error, run } = useApiPromise();
  const [popUp, setPopUp] = useState(false);
  const [responseData, setResponseData] = useState(null);
  const BASE_URL = import.meta.env.VITE_BASE_URL;
  const [letterType, setLetterType] = useState('newproposalletter');
  const [letterState, setLetterState] = useState(false);
  const { setLetterList } = useLetterList();
  ...
  useEffect(() => {
    if (letterType === "existingproposalletter") {
      setLetterState(true);
    } else {
  ...
  const filteredProposals = useMemo(() => {
    return proposalList.filter(
      (e) => letterState ? e.status.toLowerCase() !== 'initiated' : e.status.toLowerCase() === "initiated"
    );
  }, [proposalList, letterState]);
  ...
  const columns = [
      ...
      renderCell: (params) => (
        <div className='h-full w-full flex items-center  '>
          ...
          <input
            className='size-4'
            type='checkbox'
            checked={selectedProposals.some((p) => p.id === params.row.id)}
    ...
    { field: "fund_source", headerName: "Fund Source", flex: 1 },
    {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (formData.proposals.length === 0) return;

    const res = await run(() => api.post('/admin/letters/newletter', formData), "Lettter created successfully.");
    getProposal();
    setPopUp(true);
    setLetterList((prev) => [res.data.letter, ...prev]);
    setResponseData(res);
    window.open(`${BASE_URL}${res.data.filePath}`, "_blank");
    ...
    <section id='proposal_letter' className="mb-10 mt-2" >
      <div className='flex justify-between items-center'>
        ...
        <div className='border border-primary text-primary flex  bg-white justify-between rounded-[2rem]  text-xs overflow-hidden select-none'>
          <div
            onClick={() => setLetterType("newproposalletter")}
            className={`px-5 py-1 cursor-pointer font-bold ${!letterState ? "bg-primary text-white " : ""}`}>New</div>
          {/* <div className="w-[1px] bg-gray-400"></div> */}
          <div
            onClick={() => setLetterType("existingproposalletter")}
            className={`px-2 py-1 cursor-pointer font-bold  ${letterState ? "bg-primary text-white " : ""}`}>Existing</div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className='  '>
          ...
          <Table columns={columns} rows={filteredProposals} />
        </div>
